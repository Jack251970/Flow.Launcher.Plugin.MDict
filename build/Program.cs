...
public sealed class BuildTask : FrostingTask<BuildContext>
{
    ...
    {
        var projects = context.DefaultSln.Value.Projects.Where(p => p.Name.EndsWith("MDict"));
...
public class PublishTask : FrostingTask<BuildContext>
{
    public override void Run(BuildContext context)
    {
        var project = context.DefaultSln.Value.Projects.First(p => p.Name.EndsWith("MDict"));
        ...
        context.ZipCompress(
            rootPath: srcDir,
            outputPath: $"{dstDir}/MDict-{context.PublishVersion}.zip",
            filePaths: files,
            level: 9
        );
    }
